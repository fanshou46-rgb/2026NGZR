...
CandidatePlan RDFW::BuildCandidatePlan(std::size_t candidate_task_index) {
    ...
    if (!tasks[candidate_task_index].hasMissingObjects) {
...
CandidatePlan RDFW::BuildSyntheticPutOnCandidate(unsigned int object_id,
    ...
    if (object_id >= objects.size() || target_id >= objects.size() ||
        !objects[object_id] || !objects[target_id]) return CandidatePlan();
    ...
    tasks.pop_back();
    result.task_label = "multi-goto-puton";
    return result;
}
                    ...
                    const std::vector<CandidatePlan>& candidates)
{
    ...
    if (!HasExecutableCandidate(candidates)) {
        bool has_deferred_goto = false;
        if (!include_goto) {
            for (std::size_t i = 0; i < tasks.size(); ++i) {
                if (tasks[i].isEnable && !tasks[i].hasMissingObjects &&
                    tasks[i].behave == "goto") {
                    has_deferred_goto = true;
                    break;
                }
            }
        }
        const bool no_global_candidate = include_goto || !has_deferred_goto;
        LOG(YELLOW "[3B][StopGate] no complete candidate plan fits before %s; "
                   "candidate_count=%zu deferred_goto=%s\n" RESET,
            phase, candidates.size(), BoolString(has_deferred_goto));
...
bool RDFW::DryRunActionSucceeds(
    ...
    if (action == "Move")
        return !arguments.empty() && static_cast<int>(arguments[0]) >= 0;
...
void RDFW::ExecuteMainTaskLoop(bool defer_multi_goto)
{
    ...
    for (task_index = 0; task_index < tasksSize; ++task_index)
    {
        ...
        BeginCandidateExecution(*selected_candidate);
    ...
    if(stage==1){
        ...
        int flag=0;
         for(int i=0;i<tasks.size();i++){
               ...
               BeginCandidateExecution(candidate);
        ...
        if (move_set.empty()) {
            if (location != chosen_loc) {
                ...
                if (CanStartPlan(final_move, "multi-goto-final-move")) {
                    BeginCandidateExecution(final_move);
            ...
            for (unsigned id : move_set) {
                ...
                BeginCandidateExecution(relocation);
...
bool RDFW::TakeOut(unsigned int a, unsigned int b)
{
    ...
    if (shadow_dry_run ? DryRunActionSucceeds("TakeOut", arguments)
                       : Plug::TakeOut(a, b))
    {
        small->inside = NONE;
...
bool RDFW::PutIn(unsigned int a, unsigned int b)
{
    ...
    if (shadow_dry_run ? DryRunActionSucceeds("PutIn", arguments)
                       : Plug::PutIn(a, b))
    {
...
bool RDFW::Close(unsigned int a)
{
    ...
    if (shadow_dry_run ? DryRunActionSucceeds("Close", arguments)
                       : Plug::Close(a))
    {
...
bool RDFW::Open(unsigned int a)
{
    ...
    if (shadow_dry_run ? DryRunActionSucceeds("Open", arguments)
                       : Plug::Open(a))
    {
...
bool RDFW::FromPlate(unsigned int a)
{
    ...
    if (shadow_dry_run ? DryRunActionSucceeds("FromPlate", arguments)
                       : Plug::FromPlate(a))
    {
        SetHold(plate);
        SetPlate(nullptr);
...
bool RDFW::ToPlate(unsigned int a)
{
    ...
    if (shadow_dry_run ? DryRunActionSucceeds("ToPlate", arguments)
                       : Plug::ToPlate(a))
    {
        SetPlate(hold);
        SetHold(nullptr);
...
bool RDFW::PutDown(unsigned int a)
{
    ...
    if (shadow_dry_run ? DryRunActionSucceeds("PutDown", arguments)
                       : Plug::PutDown(a))
    {
        SetHold(nullptr);
        auto small = dynamic_pointer_cast<SmallObject>(objects[a]);
...
bool RDFW::PickUp(unsigned int a)
{
    ...
    if (shadow_dry_run ? DryRunActionSucceeds("PickUp", arguments)
                       : Plug::PickUp(a))
    {
        SetHold(small);
        small->location = location;
