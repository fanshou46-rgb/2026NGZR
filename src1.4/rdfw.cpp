            ...
            !cons.X.empty() && cons.X[0] && !cons.Y.empty() && cons.Y[0])
        {
            ...
            if (objects[x]) {
                if (auto sm = std::dynamic_pointer_cast<SmallObject>(objects[x])) {
                    ...
                    sm->inside = y;
                    EnsureEvidenceCapacity(static_cast<unsigned int>(x));
                    objectInsideVerified[x] = true; // official must holds in the initial state
                    // 添加到目标容器（避免重复）
                    if (objects[y]) {
...
void RDFW::ApplyOpenCloseCorrection() {
    ...
    for (const auto& obj : objects) {
        ...
        if (must_open || must_closed || cons_not_open || cons_not_closed) {
            EnsureEvidenceCapacity(cont->id);
            containerStateVerified[cont->id] = true;
        }
    }
}
