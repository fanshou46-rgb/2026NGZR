...
本目录完整复制 `scr1.3`，保留 must-near 修复，并增加自然语言词典的 CRLF 兼容处理。
...
- 读取 `words.txt` 时去除 Windows CRLF 留下的行尾 `\r`，保证 Linux/WSL 下词类标题和词条能够正确匹配。
- 仓库根目录通过 `.gitattributes` 固定各版本 `words.txt` 使用 LF。
- 除上述词典兼容处理外，程序逻辑与 `scr1.3` 一致。

以下内容记录本项目最初导入的上游历史基线，仅用于追溯；其中哈希不是 scr1.3.1 当前文件哈希。
