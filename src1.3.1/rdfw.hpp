...
namespace _home
{
    ...
    private:
        ...
        std::chrono::milliseconds EstimateTaskTime(const Instruction& task) const;
        std::chrono::milliseconds EstimateMultiGotoTime() const;
        bool HasExecutableCandidate(bool include_goto) const;
        bool StopGate(const char* phase, bool include_goto);
        bool CanStartPlan(const Instruction& task, const char* phase) const;
        ...
        void EnsureEvidenceCapacity(unsigned int id);
