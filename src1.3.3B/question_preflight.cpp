...
bool RDFW::RunQuestionPreflight() {
    ...
    for (std::size_t i = 1; i < objects.size(); ++i) {
        ...
        if (!IsModelledObject(object))
            world_error("object lacks stable sort/runtime type: " + std::to_string(i));
        ...
        if (object->sort == "human") {
            ...
            if (i != 1 || !big || std::dynamic_pointer_cast<Container>(object))
                world_error("human must be the non-container big object 1");
        }
        ...
        if (stage == 1 && big && object->location != UNKNOWN &&
            ...
            world_error("multiple big objects at Stage 1 location " +
                        std::to_string(object->location));
    }
    if (humans != 1) world_error("world must have exactly one human");
